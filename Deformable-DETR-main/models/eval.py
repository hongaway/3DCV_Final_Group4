...
def cal_add_cuda(pred_RT, gt_RT, p3ds):
    # print("pred_RT.shape", pred_RT.cpu().numpy())
...
def eval_metric(cls_ids, pred_pose_lst, pred_cls_ids, RTs):
    n_cls = 22
    cls_add_dis = [list() for i in range(n_cls)]
    cls_adds_dis = [list() for i in range(n_cls)]
    for icls, cls_id in enumerate([cls_ids.item()]):
        if cls_id == 0:
            break

        cls_idx = np.where(pred_cls_ids.cpu().numpy() == cls_id)[0]
        if len(cls_idx) == 0:
            ...
            pred_RT = torch.from_numpy(pred_RT.cpu().numpy().astype(np.float32)).cuda()
...
def eval_one_frame_pose(
    ...
    # print("pred_pose_lst = ", pred_pose_lst)
    cls_add_dis, cls_adds_dis = eval_metric(cls_ids, pred_pose_lst, pred_cls_ids, RTs)
    return (cls_add_dis, cls_adds_dis)
