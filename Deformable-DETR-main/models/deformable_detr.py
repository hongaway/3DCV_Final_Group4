...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, num_classes, matcher, weight_dict, losses, cld, focal_alpha=0.25):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if log:
            ...
            losses['class_error'] = 100 - accuracy(src_logits[idx], target_classes_o)[0] # no this part in T6D
            ################################### in T6D only #######################################
    ...
    def rotation_6d_to_matrix(self, rot_6d):
        ...
        rot_matrix = torch.cat((x.view(-1, 3, 1), y.view(-1, 3, 1), z.view(-1, 3, 1)),
                               2)  # Rotation Matrix lying in the SO(3)
    ...
    def evals(self, outputs, targets, indices):
        ...
        src_trans = outputs['pred_trans'][idx]
        source_RTs = torch.cat(
            [torch.cat((orien, trans.view(3, -1)), dim=1) for (orien, trans) in zip(src_orien, src_trans)], dim=0).view(
            -1, 3, 4)
    ...
    def forward(self, outputs, targets, eval=False): # is different from the original def from deformable detr below
...
def build(args):
    ...
    ################################### in T6D only #######################################
    losses = ['labels', 'boxes', 'cardinality', 'poses'] # instead of the above
    #######################################################################################
    ...
    criterion = SetCriterion(num_classes, matcher, weight_dict, losses, cld=cld, focal_alpha=args.focal_alpha) # in T6D
